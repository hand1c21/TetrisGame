...
import 'package:tetris_game/tile.dart';
import 'piece.dart';
...
List<List<values.TetrisShape?>> gameBoard =
...
class _GameBoardState extends State<GameBoard> {
  ...
  void startGame() {
    ...
    gameLoop(Duration(milliseconds: 700)); // מהירות
  }
  ...
  void showGameOverDialog() {
    showDialog(
        ...
        builder: (context) => AlertDialog(
              title: Text('Gmae Over'),
              content: Text('Your Score is:$currentScore'),
              actions: [
                TextButton(
                  onPressed: () {
                    // resetGame();
                    Navigator.of(context).pop((context)=>startGame());
                  },
  ...
  void createNewPiece (){
    ...
    values.TetrisShape.values[Random().nextInt(values.TetrisShape.values.length)]);
